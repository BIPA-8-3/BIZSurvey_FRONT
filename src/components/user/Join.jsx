...
function Join({ onSubmit = async (data) => {
    ...
    <div id={style.joinWrap} className={`fade-in ${fadeIn ? 'active' : ''}`}>
     <form onSubmit={handleSubmitFunction}>
        ...
        {errors.number && (
          ...
          <div className={style.inputBtn} onClick={isButtonDisabled ? null : handleCode}>인증번호 확인</div>
        </div>

        <label htmlFor="password">비밀번호<span className={style.labelSpan}>*</span></label>
        {errors.password && <small role="alert" className={style.joinError}>{errors.password.message}</small>}
        <div className={style.inputWrap}>
        ...
        <label htmlFor="passwordConfirm">비밀번호 확인<span className={style.labelSpan}>*</span></label>
        ...
        {errors.name && <small role="alert" className={style.joinError}>{errors.name.message}</small>}
        <div className={style.inputWrap}>
          <div className={`${style.inputDiv} ${style.input100}`}>
            <input
                  type="text"
        ...
        {errors.nickname && <small role="alert" className={style.joinError}>{errors.nickname.message}</small>}
        <div className={style.inputWrap}>
          <div className={style.inputDiv}>
            <input
                type="text"
          ...
          <div className={style.inputBtn} onClick={handleNickname}>중복확인</div>
        </div>

        <label htmlFor="birthdate">생년월일</label>
        <div className={style.inputWrap}>
          <div className={`${style.inputDiv} ${style.input100}`}>
              <input type='date' id='birthdate' name='birthdate' className={style.input}
              {...register('birthdate')}/>
          </div>
        </div>

        <label htmlFor="gender">성별<span className={style.labelSpan}>*</span></label><br />
        <FormControl>
          <RadioGroup row aria-labelledby="demo-row-radio-buttons-group-label" name="gender" id="gender" defaultValue="MALE"
          >
              <FormControlLabel value="MALE" control={<Radio defaultChecked/>} label="남자" 
              {...register('gender')}/>
