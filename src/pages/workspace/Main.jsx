...
export default function Main() {
  ...
  useEffect(() => {
    const newEventSource = new EventSource(getURI() + "/sse/connect?userId=" + userInfo.id);
    ...
    newEventSource.addEventListener("connect", function (event) {
      console.log(event.data);
    });

    newEventSource.addEventListener("accept", function (event) {
      ...
      console.log(owner);
      if (data && data.userId && owner.workspaceId === data.workspaceId) {
        console.log("acceptInvite: ", data);
        setAdminList([...adminList, data]);
        const tempList = adminWaitList.filter((a) => a.email !== data.email);
        setAdminWaitList([...tempList]);
      }
    });
...
//       console.log("워크스페이스 변경");
//       if (eventSource) {
//         console.log("워크스페이스 변경 - 1");
//         eventSource.close();
//         setEventSource(null);
//         console.log("워크스페이스 변경 - 1 - 1");
//       }

//       if (selectedWorkspaceId === 2) {
//         // SSE 연결 설정
//         const newEventSource = new EventSource(
//           getURI() + "/sse/connect?workspaceId=" + selectedWorkspaceId
//         );
...
//         newEventSource.onerror = (event) => {
//           console.log("왜 안옴, " + event.eventPhase);
//           if (event.eventPhase === EventSource.CLOSED) {
//             console.error("SSE 연결이 닫혔습니다.");
//           } else if (event.eventPhase === EventSource.error) {
//             console.error("SSE 오류 발생");
...
//         newEventSource.addEventListener("accept", function (event) {
//           var data = JSON.parse(event.data);
//           if (data && data.userId && selectedWorkspaceId === data.workspaceId) {
//             console.log("acceptInvite: ", data);
//             setAdminList([...adminList, data]);
//             const tempList = adminWaitList.filter((a) => a.email !== data.email);
//             setAdminWaitList([...tempList]);
